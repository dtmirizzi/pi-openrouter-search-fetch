...
describe("statusLabel (verbose mode — default)", () => {
  ...
  it("custom engines", () => {
    expect(statusLabel({ ...base, searchEngine: "exa", fetchEngine: "firecrawl" })).toBe(
      "search:on(exa) | fetch:on(firecrawl)",
    );
  });
  it("all toggles on", () => {
    expect(
      ...
      "search:on(auto) | fetch:on(auto) | img:on(gemini-2.5-flash-image-preview) | vision:on(gemini-2.5-flash) | video:on(gemini-2.5-flash) | pdf:on(gemini-2.5-flash) | tts:on(gpt-4o-mini-tts) | stt:on(whisper-large-v3)",
    );
  });
  it("only new tools on", () => {
    expect(
      statusLabel({ ...base, searchEnabled: false, fetchEnabled: false, visionEnabled: true, pdfEnabled: true }),
...
describe("statusLabel (compact mode)", () => {
  ...
  it("all toggles on", () => {
    expect(
      ...
      "S auto | F auto | Img:gemini-2.5-flash-image-preview | Vis:gemini-2.5-flash | Vid:gemini-2.5-flash | PDF:gemini-2.5-flash | TTS:gpt-4o-mini-tts | STT:whisper-large-v3",
    );
  });
});
